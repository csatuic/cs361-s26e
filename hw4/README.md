...
Starting with `server.c`, write a program that matches the `./test_server.sh` functionality. You will need to
create an `AF_INET` socket of type `SOCK_STREAM` with protocol `0` using `socket()`. 
Then `bind()` it to the correct address `127.0.0.1` and port. The 16-bit port number is specified in big-endian
in internet protocols, so use `htons()` to reverse the order of the bytes in your port number.
...
The `limerick` program from `hw3` now includes a networking feature. Given a `-s` argument, it will connect to a server, then do the following for a random number of iterations: either present a limerick to the server, or 
receive a limerick from the server. Build the `limerick` program in the `hw3/limerick` folder as in homework 3.
